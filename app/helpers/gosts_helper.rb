module GostsHelper
  ...
  A = arrA.map {|s| ([s].pack("H*")).unpack("Q>")[0]}
  ...
  C = arrC.map {|s| ([s].pack("H*")).reverse}

  def PS data
    value = Array.new(64, 0)
    (0..BLOCKSIZE-1).each do |i|
      value[Tau[i]] = Sbox[data[i].ord]
    end
    return value.map {|x| x.chr}.join("")
  end

  def L data
    value = []
    (0..7).each do |i|
      val = data[i * 8, 8].unpack("<Q")[0]
      kq = 0
      (0..BLOCKSIZE-1).each do |j|
        kq ^= A[j] if (val & 0x8000000000000000 != 0)
        val <<= 1
      end
      value << [kq].pack("<Q")
    end
  ...
  def strxor str1, str2
    a = str1.split("")
    b = str2.split("")
    a.zip(b).map {|i, j| (i.ord ^ j.ord).chr}.join("")
  end
  ...
  def g n, h, m
    value = E(LPS(strxor(h[0,8], [n].pack("<Q")) + h[8..h.size]), m)
    strxor(strxor(value, h), m)
  end
  ...
  def addEC a, p, p1, p2
    ...
    if x1 == x2
      lamda = (3 * x1 * x1 + a) * invert(2 * y1, p) % p
    else
      lamda = (y2 - y1) * invert(x2 - x1, p) % p
    end
    x = (lamda * lamda - x1 - x2) % p
    y = (lamda * (x1 - x) - y1) % p
    q = [x , y]
    return q
  end
